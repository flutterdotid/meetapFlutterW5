...
import '../model/RecordList.dart';
import '../model/RecordService.dart';
import './newEmpDetail.dart';
import '../helper/config.dart' as appConfig;

class newEmp_old extends StatefulWidget {

  @override
  _newEmp_oldState createState() {
    return _newEmp_oldState();
  }
}

class _newEmp_oldState extends State<newEmp_old> {
  ...
  Widget _appBarTitle = new Text(appConfig.strings.app_title);
  ...
  Widget _buildBar(BuildContext context) {
    return new AppBar(
        ...
        backgroundColor: appConfig.colors.ColorPrimaryDark,
        centerTitle: true,
        title: Text(appConfig.strings.app_title),
    ...
    return Card(
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          onTap: () {
            Navigator.push(
                context, MaterialPageRoute(builder: (context) => new newEmpDetail(record: record)));
          },
        ),
      ),
    );
  ...
  void _searchPressed() {
    setState(() {
      if (this._searchIcon.icon == Icons.search) {
        ...
        this._appBarTitle = new Text('filter');
        _filter.clear();
      }
    });
  }
